...
def download_order_report(page, start_date=None, end_date=None):
    ...
    # "다운로드 서식 선택" 드롭다운을 "출고확인용"으로 변경
    page.get_by_text("기본서식", exact=True).first.click()
    page.wait_for_timeout(500)
    page.get_by_text("출고확인용", exact=True).click()
    page.wait_for_timeout(500)
    page.screenshot(path="order_2_excel_popup_configured.png", full_page=True)
    ...
    try:
        ...
        print("발주 다운로드가 시작되지 않음 (0건으로 추정) - 건너뜀")
        return None
...
def run():
    ...
    with sync_playwright() as p:
        ...
        for attempt in range(1, 4):
            ...
            try:
                print(f"[시도 {attempt}/3] 발주 수집 시작")
                login(page)
                download_order_report(page)  # 0건이면 None 반환하고 정상 종료
                page.close()
                browser.close()
                ...
                try:
                    page.screenshot(path=f"order_error_attempt{attempt}.png", full_page=True)
                except Exception:
                    pass
                page.close()
                if attempt < 3:
